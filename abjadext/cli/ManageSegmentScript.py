...
class ManageSegmentScript(ScorePackageScript):
    ...
    def _collect_matching_paths(self, names, include_unstaged):
        ...
        for name in names:
            ...
            for path in sorted(self._segments_path.glob(name)):
                if path not in valid_paths:
                    continue
                elif path in staged_matching_paths or path in unstaged_matching_paths:
                    continue
        ...
        matching_paths.extend(
            sorted(
                staged_matching_paths, key=lambda path: staged_names.index(path.name)
            )
        )
    ...
    def _handle_collect(self):
        ...
        with open(str(segments_ily_path), "w") as file_pointer:
            ...
            for name in staged_names:
                ...
                file_pointer.write(include_template.format(name=name, sep=os.path.sep))
            file_pointer.write("}\n")
    ...
    def _handle_edit(self, segment_name):
        ...
        matching_paths = self._collect_matching_paths(
            globbable_names, include_unstaged=True
        )
    ...
    def _handle_illustrate(self, segment_name, unstaged=False):
        ...
        matching_paths = self._collect_matching_paths(
            globbable_names, include_unstaged=unstaged
        )
        if not matching_paths:
            print("    No matching segments.")
            self._handle_list()
    ...
    def _handle_list(self):
        ...
        staged_names = [_ for _ in self._read_segments_list_json() if _ in valid_names]
    ...
    def _handle_render(self, segment_name, unstaged=False):
        ...
        matching_paths = self._collect_matching_paths(globbable_names, unstaged)
        if not matching_paths:
            print("    No matching segments.")
            self._handle_list()
        for path in matching_paths:
    ...
    def _handle_stage(self):
        ...
        all_names = [
            path.name
            for path in self._list_segment_subpackages(self._score_package_path)
        ]
    ...
    def _illustrate_one_segment(self, segment_directory):
        ...
        if previous_segment_name:
            previous_segment_metadata_path = self._segments_path.joinpath(
                previous_segment_name, "metadata.json"
            )
        ...
        metadata["first_bar_number"] = previous_metadata.get(
            "measure_count", 0
        ) + previous_metadata.get("first_bar_number", 1)
        segment_package_path = self._path_to_packagesystem_path(segment_directory)
        definition_import_path = segment_package_path + ".definition"
        try:
            module = self._import_path(
                definition_import_path, self._score_repository_path
            )
        ...
        ly_path = self._write_lilypond_ly(
            lilypond_file, output_directory=segment_directory
        )
        self._write_lilypond_pdf(ly_path=ly_path, output_directory=segment_directory)
    ...
    def _process_args(self, arguments):
        ...
        if arguments.illustrate is not None:
            self._handle_illustrate(
                segment_name=arguments.illustrate, unstaged=arguments.unstaged
            )
        if arguments.list_:
            self._handle_list()
        if arguments.new:
        ...
        if arguments.render is not None:
            self._handle_render(
                segment_name=arguments.render, unstaged=arguments.unstaged
            )
        if arguments.stage:
            self._handle_stage()
    ...
    def _process_illustration_contents(self, ly_path):
        ...
        while contents and contents[0] != r"\score { %! abjad.LilyPondFile":
            contents.pop(0)
        contents.pop(0)
        for i in range(len(contents)):
            if contents[i] == "} %! abjad.LilyPondFile":
                contents = contents[:i]
                break
        contents = "\n".join(contents)
        return contents
    ...
    def _setup_argument_parser(self, parser):
        ...
        action_group.add_argument(
            "--new", "-N", help="create a new segment", metavar="NAME"
        )
        action_group.add_argument(
            "--edit", "-E", help="edit segments", metavar="PATTERN", nargs="*"
        )
        action_group.add_argument(
            "--illustrate",
            "-I",
        ...
        action_group.add_argument(
            "--collect", "-C", help="collect segment illustrations", action="store_true"
        )
        action_group.add_argument(
            "--stage", "-T", help="stage segments for illustration", action="store_true"
        )
        action_group.add_argument(
            "--list", "-L", dest="list_", help="list segments", action="store_true"
        )
        #        action_group.add_argument(
        #            '--copy', '-Y',
        #            help='copy segment',
        ...
        common_group.add_argument(
            "-u",
            "--unstaged",
            help="Include segments not staged in segments{sep}metadata.json".format(
                sep=os.path.sep
            ),
            action="store_true",
        )
