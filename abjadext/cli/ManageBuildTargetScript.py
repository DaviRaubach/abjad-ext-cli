...
class ManageBuildTargetScript(ScorePackageScript):
    ...
    def _handle_create(self, target_name, force, paper_size=None, orientation=None):
        ...
        message = "Creating build target {!r} ({}{} x {}{})".format(
            name, dimensions[0][0], dimensions[0][1], dimensions[1][0], dimensions[1][1]
        )
        print(message)
    ...
    def _handle_distribute(self, build_target_name):
        ...
        target_names = [
            path.name
            for path in sorted(self._build_path.glob("*"))
    ...
    def _handle_render(
        ...
        if render_score:
            ...
            paths_to_open.append(path)
        if any([render_preface, render_score, render_front_cover, render_back_cover]):
    ...
    def _render_back_cover(self, build_target_path):
        ...
        if not path.is_file():
            print(
                "    Missing: {!s}".format(path.relative_to(self._score_package_path))
            )
            sys.exit(1)
    ...
    def _run_lilypond(self, lilypond_path):
        command = "{} -dno-point-and-click -o {} {}".format(
            "lilypond", str(lilypond_path).replace(".ly", ""), str(lilypond_path)
        )
        with abjad.system.TemporaryDirectoryChange(str(lilypond_path.parent)):
            exit_code = subprocess.call(command, shell=True)
        if exit_code:
            print(
                "    Failed to render: {!s}".format(
    ...
    def _setup_argument_parser(self, parser):
        ...
        action_group.add_argument(
            "--render", "-R", help="render sources", metavar="NAME"
        )
        action_group.add_argument(
            "--distribute",
            "-U",
        ...
        action_group.add_argument(
            "--list", "-L", dest="list_", help="list build targets", action="store_true"
        )
        #        action_group.add_argument(
        #            '--copy', '-Y',
        ...
        render_group = parser.add_argument_group(
            "--render flags", "Use when rendering specific assets only."
        )
        ...
        render_group.add_argument(
            "--preface", help="render the preface LaTeX source", action="store_true"
        )
        render_group.add_argument(
            "--score",
        ...
        render_group.add_argument(
            "--music", help="render the music LilyPond source", action="store_true"
        )
        render_group.add_argument(
            "--parts", help="render the parts LilyPond source", action="store_true"
        )
